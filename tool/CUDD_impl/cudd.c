...
char *seperate_variables(char *infix, char ***varNames, char ***vars_row, int *size_of_vars, int *vars_row_size)
{
    ...
    temp_name = (char *) malloc(sizeof(char) * 20); // maximum variable name size is 20 //
    for(i = 0; i < strlen(infix); i++)
    {
        if(identify_symbol(infix[i]) == 0 || identify_symbol(infix[i]) == -1) // is variable or number //
        {
            temp_name[pos] = infix[i];
            pos++;
        }
...
int find_same_nodes(DdNode **vars, DdNode *node, int size)
{
    int i;
    for(i = 0; i < size; i++)
    {
...
DdNode *concat_bdds(DdNode **vars, char **varNames, char **vars_row, char *postfix, int vars_size, int vars_row_size)
{
    ...
    for(i = 0; i < strlen(postfix); i++)
    {
        ...
        else if (result == 3)   // operator * //
        {
            ...
            temp_bdd[temp_bdd_pos] = Cudd_bddAnd(gbm, get_node_1, get_node_2);

            Cudd_Ref(temp_bdd[temp_bdd_pos]);

            if(find_same_nodes(vars, get_node_1, vars_size) == 0)
            {
                Cudd_RecursiveDeref(gbm, get_node_1);
            }

            if(find_same_nodes(vars, get_node_2, vars_size) == 0)
            {
                Cudd_RecursiveDeref(gbm, get_node_2);
            }
